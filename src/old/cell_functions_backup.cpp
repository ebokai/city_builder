...
void Game::cell_dynamics_less(int size, int x1, int x2, int x3, int t, float p){

	// cout << "CL: " << x1 << " " << x2 << " " << x3 << endl;
...
void Game::cell_dynamics_two(int size, int x1, int x2, int x3, int x4, int t, float p){
	...
	if ((this_cell == x1) && (types[x2] > t) && (types[x3] > t) && (u < p)){
		grid_states[i][j] = x4;
	}
}